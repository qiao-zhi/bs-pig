...
function showTable(pageInfo) {
    ...
    for(var i=0,length_1 = users.length;i<length_1;i++){
        ...
        var tr = "<tr>"
            +'<td>'+index+'</td>'
            +'<td>'+replaceNull(users[i].shippingUnit)+'</td>'
            +'<td>'+replaceNull(users[i].purchaseUnit)+'</td>'
            +'<td>'+replaceNull(users[i].tradeTime)+'</td>'
            +'<td>'+replaceNull(users[i].tradeAddress)+'</td>'
            +'<td>'+replaceNull(users[i].orderNumbers)+'</td>'
            +'<td>';
	        if(isAdmin()){
