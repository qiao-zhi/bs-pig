var pageUrl = "/charge/page.html";
var deleteUrl = "/charge/delete.html";
var updateUrl = "/charge/update.html";
...
function showTable(pageInfo) {
    ...
    for(var i=0,length_1 = users.length;i<length_1;i++){
        ...
        var tr = "<tr>"
            +'<td>'+index+'</td>'
            +'<td>'+replaceNull(users[i].fee)+'</td>'
            +'<td>'+replaceNull(users[i].refuse)+'</td>'
            +'<td>'+replaceNull(users[i].taxi)+'</td>'
            +'<td>';
	        if(isAdmin()){
